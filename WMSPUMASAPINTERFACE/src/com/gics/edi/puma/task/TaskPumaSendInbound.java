...
public class TaskPumaSendInbound extends TimerTask {
	...
	public void run() {
		...
		sbQuery.append(" SELECT INBOUND_NO ")
		.append(" FROM LI020NM ")
		.append(" WHERE CENTER_CD = 'A1' ") 
		.append(" AND INBOUND_STATE = '60' ") 
		.append(" AND ( SEND_STATE = '00' OR SEND_STATE IS NULL ) ") 
		.append(" AND BRAND_CD = '6101' ") 
		.append(" AND INBOUND_DATE = TO_DATE(TO_CHAR(SYSDATE,'YYYYMMDD'),'YYYYMMDD') ");
		...
		try {
			...
			for(int i=0; i<outboundList.size(); i++) {
				ArrayList<String[]> sendData = linker.createSendData("sendInbound", "puma", outboundList.get(i));
